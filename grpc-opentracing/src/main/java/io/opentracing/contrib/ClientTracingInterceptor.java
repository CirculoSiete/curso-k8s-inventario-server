...
public class ClientTracingInterceptor implements ClientInterceptor {
    ...
    public <ReqT, RespT> ClientCall<ReqT, RespT> interceptCall(
        ...
        for (ClientRequestAttribute attr : this.tracedAttributes) {
            switch (attr) {
                case AFFINITY:
                    CallOptions.Key<String> affinityKey = CallOptions.Key.of("affinity", null);
                    String affinity = callOptions.getOption(affinityKey);
                    if (affinity == null) {
                        span.setTag("grpc.affinity", "null");
                    } else {
                        span.setTag("grpc.affinity", affinity);
                    }  
                    break;
